<template>
  <q-page>
      ...
      <template #selects>
        <div class="flex" :class="{'middle-modificator': !active}">
          <q-select
            ...
            :hide-dropdown-icon="!isNeedSelect || (typeof isNeedSelect === 'string' && isNeedSelect.indexOf('plugins') > -1)"
            :label="active ? 'Plugin' : 'SELECT PLUGIN'"
            dark hide-bottom-space dense color="white"
            :disable="!isNeedSelect || (typeof isNeedSelect === 'string' && isNeedSelect.indexOf('plugins') > -1)"
            ...
            @filter="(filter, update) => filterItems('plugins', filter, update)"
          >
          ...
          <transition appear enter-active-class="animated bounceInDown" leave-active-class="animated bounceOutUp" v-if="$q.platform.is.desktop && selectedItem && !selectedItem.deleted">
            <q-btn key="plugin" title="View plugins" class="on-right pull-right text-center rounded-borders q-px-xs q-py-none text-white" style="width: 60px;" @click="goToPlugin" flat dense>
              <q-icon size="1.5rem" color="white" name="mdi-call-split"/>
              <div style="font-size: .7rem; line-height: .7rem;">Plugin</div>
            </q-btn>
          </transition>
        </div>
      </template>
    ...
    <traffic-viewer
      ...
      class="flex"
    />
    <div v-if="!items.length && isItemsInit" class="text-center text-grey-3 q-mt-lg" style="font-size: 2rem;">{{isLoading ? 'Fetching data..' : 'Plugins not found'}}</div>
  </q-page>
</template>

<script>
import TrafficViewer from '../../components/trafficViewer/plugins/Index.vue'
...
export default {
  ...
  computed: {
    ...
    items () {
      return Object.values(this.mainStore.plugins || {})
    },
    filteredItems () {
  ...
  methods: {
    ...
    goToPlugin () {
      if (this.prevEntity === 'plugins' && this.prevRoute && this.prevRoute.params.id === this.active) {
        this.$router.push(this.prevRoute).catch(err => err)
      } else {
        this.$router.push(`/plugins/${this.active}`).catch(err => err)
      }
    },
    init () {
      const entity = 'tools/pluginTraffic'
  ...
  watch: {
    ...
    active (val) {
      ...
      if (val) {
        this.$emit('update:settings', { type: 'ENTITY_CHANGE', opt: { entity: 'tools/pluginTraffic' }, value: currentItem.id })
      }
    }
  },
  components: {
