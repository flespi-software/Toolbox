<template>
  <q-page>
      ...
      <template #selects>
        <div style="max-width: 50%" :class="{'middle-modificator': !active}">
          <q-select
            ...
            :label="active ? 'Group' : 'SELECT GROUP'"
    ...
    <div v-if="!items.length && isItemsInit" class="text-center text-grey-3 q-mt-lg">
      <div style="font-size: 2rem;">{{isLoading ? 'Fetching data..' : 'Groups not found'}}</div>
...
export default {
  ...
  computed: {
    ...
    itemsCollection () {
      return this.mainStore.groups || {}
    },
    // protocols () { return this.mainStore.streamsProtocols }
    items () {
