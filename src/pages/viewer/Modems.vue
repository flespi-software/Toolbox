<template>
  <q-page>
    <q-toolbar class="justify-between bg-grey-9">
      <div style="max-width: 50%" :class="{'middle-modificator': !active}" v-if="items.length">
        <!-- <q-item class="no-padding" :style="{cursor: isNeedSelect ? '' : 'default!important'}">
          <q-item-section :title="active && (selectedItem.configuration && selectedItem.configuration.protocol)">
            <q-item-label header class="ellipsis overflow-hidden" :style="{maxWidth: '140px'}">{{active ? selectedItem.name || '&lt;noname&gt;' : 'SELECT MODEM'}}</q-item-label>
            <q-item-label caption style="font-size: 0.8rem" v-if="active && selectedItem.configuration && selectedItem.configuration.uri">{{selectedItem.configuration.uri}}</q-item-label>
          </q-item-section>
          <q-item-section class="text-right">
            <q-item-label style="display: inline-block" stamp color="white" class="text-center" v-if="active"><div v-if="selectedItem.deleted" class="cheap-modifier"><small>DELETED</small></div>#{{selectedItem.id.toString()}}</q-item-label>
              ...
              >
                ...
                >
                  <q-item-section>
                    ...
                    <q-item-label caption v-if="item.configuration"><small>{{item.configuration.source_addr || '&lt;no address&gt;'}}</small></q-item-label>
                    <q-item-label caption v-if="item.configuration"><small>{{item.configuration.uri || '&lt;no uri&gt;'}}</small></q-item-label>
                  </q-item-section>
                  <q-item-section class="text-center">
                    <q-item-label v-if="item.deleted" class="cheap-modifier"><small>DELETED</small></q-item-label>
                    <q-item-label><small>#{{item.id.toString()}}</small></q-item-label>
                  </q-item-section>
                </q-item>
              </VirtualList>
            </q-list>
        ...
        <q-select
          ...
          filled
          :label="active ? 'Modems' : 'SELECT MODEM'"
    ...
    <div v-if="!items.length" class="text-center text-grey-3 q-mt-lg">
      <div style="font-size: 2rem;">{{isLoading ? 'Fetching data..' : 'Modems not found'}}</div>
