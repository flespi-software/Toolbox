<template>
  <q-page>
      ...
      <template #selects>
        <div style="max-width: 50%" :class="{'middle-modificator': !active}">
          <q-select
            ...
            :label="active ? 'Connectors' : 'SELECT CONNECTOR'"
    ...
    <logs
      ...
      :item="selectedItem"
      :limit="limit"
      originPattern="ai/connectors/:id"
    ...
    <div v-if="!items.length && isItemsInit" class="text-center text-grey-3 q-mt-lg">
      <div style="font-size: 2rem;">{{isLoading ? 'Fetching data..' : 'Connectors not found'}}</div>
    </div>
    <widgets
      ref="logsView"
...
export default {
  ...
  computed: {
    ...
    itemsCollection () {
      return this.mainStore.connectors || {}
    },
    items () {
  ...
  methods: {
    /* the type is what tells two connectors apart; its name would cost a separate connector-types request */
    subtitle (item) {
      return item && item.type_id ? `type #${item.type_id}` : '<no type>'
    },
    clearHandler () {
      this.$q.dialog({
        title: 'Confirm',
