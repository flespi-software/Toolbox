<template>
  <q-page>
     <q-toolbar class="justify-between bg-grey-9">
      ...
      <q-btn v-if="active && !selectedItem.deleted" flat dense class="on-right pull-right text-center rounded-borders q-px-xs q-py-none text-white" color="white" @click="modeModel = !modeModel" style="min-width: 70px; max-width: 70px;">
  ...
  watch: {
    ...
    active (val) {
      ...
      if (val) {
        ...
        this.$router.push(`/containers/${val}`).catch(err => err)
      } else {
        this.$router.push('/containers').catch(err => err)
      }
