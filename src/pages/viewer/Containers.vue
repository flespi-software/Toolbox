...
import { useContainersMessagesStore as useMessagesListStore } from 'src/qvirtualscroll/stores/containersMessages'
...
export default {
  ...
  computed: {
    ...
    itemsCollection () {
      return this.mainStore.containers || {}
    },
    items () {
    ...
    panelsWidgetsStyle () {
      const style = {}
      const isLeftSide = this.widgetStyle.left && (this.isWidgetsMessageActive || this.isWidgetsLogsActive)
      const isRightSide = this.widgetStyle.right && (this.isWidgetsMessageActive || this.isWidgetsLogsActive)
