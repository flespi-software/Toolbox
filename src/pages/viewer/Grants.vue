<template>
  <q-page>
      ...
      <template #selects>
        <div style="max-width: 50%" :class="{'middle-modificator': !active}">
          <q-select
            ...
            :label="active ? 'Grants' : 'SELECT GRANT'"
    ...
    <logs
      ...
      :item="selectedItem"
      :limit="limit"
      originPattern="platform/grants/:id"
    ...
    <div v-if="!items.length && isItemsInit" class="text-center text-grey-3 q-mt-lg">
      <div style="font-size: 2rem;">{{isLoading ? 'Fetching data..' : 'Grants not found'}}</div>
