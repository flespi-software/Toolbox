<template>
  <q-page>
    ...
    >
      <template #selects>
        <div style="max-width: 50%" class="flex" :class="{'middle-modificator': !active}">
          <q-select
            ref="itemSelect"
            class="items__select"
            ...
            :label="active ? 'Geofences' : 'SELECT GEOFENCE'"
    ...
    <div v-if="!items.length && isItemsInit" class="text-center text-grey-3 q-mt-lg">
      <div style="font-size: 2rem;">{{isLoading ? 'Fetching data..' : 'Geofences not found'}}</div>
...
export default {
  ...
  computed: {
    ...
    itemsCollection () {
      return this.mainStore.geofences || {}
    },
    // needTrafficRoute (state) {
    //   return get(state.config, 'pluginTraffic.isDrawable', false)
    // },
    items () {
  ...
  methods: {
    ...
    // trafficViewHandler () {
    //   this.$router.push(`/tools/plugin-traffic/${this.active}`).catch(err => err)
    // },
    ...
    //       to, from, highlight
    //     }
    //   this.$router.push({ path: `/tools/plugin-traffic/${this.active}`, query }).catch(err => err)
    // },
