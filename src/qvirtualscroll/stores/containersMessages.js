/*
 * The containersMessages list store — the Vuex module the Vue 2 build registered under the list's name,
...
export function createContainersMessagesStore ({ name, lsNamespace, storage, errorHandler, newMessagesInterseptor }) {
  ...
  return defineStore(`qvs-containersMessages-${name}`, {
    state: () => ({
      name,
      lsNamespace,
      isLoading: false,
    ...
    actions: {
      ...
      getParams () {
        const params = {}
        if (this.limit) {
          params.max_count = this.limit
        }
        if (this.filter) {
          params.filter = `${this.filter}`
        }
        if (this.from) {
          params.left_key = this.from / 1000
        }
        if (this.to) {
          params.right_key = this.to / 1000
        }
        if (this.reverse) {
      ...
      async initTime () {
        ...
        if (mainStore.token && this.active) {
          try {
            ...
            const resp = await connector.storage.getContainersMessages(this.active, { data: JSON.stringify(params) })
            this.reqStart({ endpoint: 'getContainersMessages-initTime', active: this.active, data: JSON.stringify(params) })
      ...
      async getMessages (params) {
        ...
        if (mainStore.token && this.active) {
          ...
          try {
            ...
            const resp = await connector.storage.getContainersMessages(this.active, { data: JSON.stringify(params) })
            this.reqStart({ endpoint: 'getContainersMessages', active: this.active, data: JSON.stringify(params) })
            const data = resp.data
            this.errorsCheck(data)
      ...
      async get (initTimestamp) {
        if (!this.isLoading) {
          ...
          const needRT = (params.right_key >= now && (this.limit && messages.length < this.limit) && !this.loopId)
          let startRTRender = () => {}
          if (needRT) {
          ...
          } else if ((params.right_key < now || (this.limit && messages.length >= this.limit)) && this.loopId) {
            await this.unsubscribePooling()
          }
      ...
      async getPrevPage () {
        if (!this.isLoading) {
          this.isLoading = true
          const to = _get(this, 'messages[0].' + keyfield, this.to) - 0.000001
          const params = this.getParams()
          params.right_key = to
          params.reverse = true
      ...
      async getNextPage () {
        if (!this.isLoading) {
          ...
          const from = _get(this, `messages[${this.messages.length - 1}].${keyfield}`, this.from) + 0.000001
          const params = this.getParams()
          let messagesCount = 0
          params.left_key = from
          const messages = await this.getMessages(params)
          messagesCount += messages.length
          const needRT = (params.right_key > Math.floor(Date.now() / 1000) && (this.limit && messages.length < this.limit) && !this.loopId)
          let startRTRender = () => {}
          if (needRT) {
            startRTRender = await this.pollingGet()
            const stop = Date.now()
...
export function useContainersMessagesStore (config) {
  return useListStore(createContainersMessagesStore, config)
}
