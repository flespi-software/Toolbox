/*
 * The channelsMessagesSerial list store — the Vuex module the Vue 2 build registered under the list's name,
...
export function createChannelsMessagesSerialStore ({ name, lsNamespace, storage, errorHandler, newMessagesInterseptor }) {
  ...
  return defineStore(`qvs-channelsMessagesSerial-${name}`, {
    state: () => ({
      name,
      lsNamespace,
      isLoading: false,
    ...
    actions: {
      ...
      async clear () {
        ...
        this.limit = 1000
        this.reverse = false
        await connector.unsubscribeMessagesChannels(this.active)
        logger.info(`clear module`)
        logger.info(`unsubscribeMessagesChannels ${this.active}`)
      },
      ...
      async getCols (sysColsNeedInitFlags) {
        ...
        if (mainStore.token && this.active) {
          try {
            ...
            colsFromStorage = (colsFromStorage && colsFromStorage[this.active])
            ...
            const protocolIdResp = await connector.gw.getChannels(this.active, { fields: 'protocol_id' })
            this.reqStart({ endpoint: 'getChannels', active: this.active, fields: 'protocol_id' })
            const protocolIdData = protocolIdResp.data
            this.errorsCheck(protocolIdData)
            if (protocolIdData.result && protocolIdData.result.length && protocolIdData.result[0].protocol_id) {
              const colsResp = await connector.gw.getChannelProtocols(protocolIdData.result[0].protocol_id, { fields: 'message_parameters' })
              this.reqStart({ endpoint: 'getChannelProtocols', active: protocolIdData.result[0].protocol_id, fields: 'message_parameters' })
              const colsData = colsResp.data
              this.errorsCheck(colsData)
      ...
      async initTime () {
        ...
        if (mainStore.token && this.active) {
          try {
            ...
            const resp = await connector.gw.getChannelsMessages(this.active, { data: JSON.stringify(params) })
            this.reqStart({ endpoint: 'getChannelsMessages-initTime', active: this.active, data: JSON.stringify(params) })
      ...
      async getMessages (params) {
        ...
        if (mainStore.token && this.active) {
          ...
          try {
            ...
            const resp = await connector.gw.getChannelsMessages(this.active, { data: JSON.stringify(params) })
            this.reqStart({ endpoint: 'getChannelsMessages', active: this.active, data: JSON.stringify(params) })
            const data = resp.data
            this.errorsCheck(data)
      ...
      async getPrevPage () {
        if (!this.isLoading) {
          this.isLoading = true
          const to = _get(this, 'messages[0]["server.timestamp"]', this.to) - 0.000001
          const params = this.getParams()
          params.to = to
          params.reverse = true
      ...
      async getNextPage () {
        if (!this.isLoading) {
          ...
          const from = _get(this, `messages[${this.messages.length - 1}]['server.timestamp']`, this.from) + 0.000001
          const params = this.getParams()
          let messagesCount = 0
          params.from = from
          const messages = await this.getMessages(params)
          messagesCount += messages.length
          const needRT = (params.to + 2 > Math.floor(Date.now() / 1000) && (this.limit && messages.length < this.limit) && !this.loopId)
          let startRTRender = () => {}
          if (needRT) {
            startRTRender = await this.pollingGet()
            const stop = Date.now()
      ...
      async pollingGet () {
        ...
        await connector.subscribeMessagesChannels(this.active, '+', (message) => {
          this.messagesBuffer.push(JSON.parse(message))
        }, { rh: 2, prefix: filter })
        this.realtimeEnabled = true
        logger.info(`subscribed to messagesChannels ${this.active} ${filter || ''}`)
        return () => {
      ...
      async unsubscribePooling () {
        ...
        const filter = this.filter ? `$filter/payload=${encodeURIComponent(this.filter)}` : undefined
        await connector.unsubscribeMessagesChannels(this.active, '+', undefined, { prefix: filter })
        this.realtimeEnabled = false
        logger.info(`unsubscribed to messagesChannels ${this.active} ${filter || ''}`)
      },
      ...
      async newMessagesCheck () {
        ...
        await connector.subscribeMessagesChannels(this.active, '+', () => {
          this.hasNewMessages = true
          this.unsubscribePooling()
        }, { rh: 2 })
        logger.info(`newMessagesCheck subscribed to messagesChannels ${this.active}`)
      },
...
export function useChannelsMessagesSerialStore (config) {
  return useListStore(createChannelsMessagesSerialStore, config)
}
