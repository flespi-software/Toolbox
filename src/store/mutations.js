      ...
      const rights = tokenInfo.access.acl.reduce((result, acl) => {
        if (acl.uri === 'gw' || acl.uri === 'storage') {
          if (acl.methods.includes('GET')) {
            let entities = []
            if (acl.uri === 'gw') entities = ['channels', 'calcs', 'intervals', 'plugins', 'devices', 'groups', 'streams', 'modems', 'protocols']
...
function getToolboxSettings (state) {
  ...
  if (!settings) {
    ...
    const entityNames = ['devices', 'channels', 'calcs', 'intervals', 'plugins', 'groups', 'streams', 'modems', 'containers', 'cdns', 'tools/hex', 'tools/traffic', 'platform', 'mqtt', 'webhooks', 'grants', 'realms']
