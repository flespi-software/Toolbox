...
async function initTime ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getPluginsPackets(state.active, params)
    Vue.$logger.info(`[pluginTraffic]getMessages`)
    commit('reqStart', { endpoint: 'getPluginsPackets', ids: state.active, params })
...
async function getMessagesTail ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getPluginsPackets(state.active, params)
    Vue.$logger.info(`[pluginTraffic]getMessagesTail`)
    commit('reqStart', { endpoint: 'getPluginsPackets', ids: state.active, params })
    const messages = get(resp, 'data.result', [])
    messages.reverse()
    commit('setMessages', messages)
...
async function getMessagesNext ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getPluginsPackets(state.active, params)
    Vue.$logger.info(`[pluginTraffic]getMessagesNext`)
    commit('reqStart', { endpoint: 'getPluginsPackets', ids: state.active, params })
...
async function getMessagesPrev ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getPluginsPackets(state.active, params)
    Vue.$logger.info(`[pluginTraffic]getMessagesPrev`)
    commit('reqStart', { endpoint: 'getPluginsPackets', ids: state.active, params })
    messages = get(resp, 'data.result', [])
    messages.reverse()
...
function pollingGetMessages ({ state, commit }) {
  mesagesPollingId = setInterval(async () => {
    try {
      ...
      const resp = await Vue.connector.gw.getPluginsPackets(state.active, params)
      Vue.$logger.info(`[pluginTraffic]pollingGetMessages`)
      commit('reqStart', { endpoint: 'getPluginsPackets', ids: state.active, params })
      const messages = get(resp, 'data.result', [])
      commit('setMessagesAppend', messages)
    } catch (e) {
...
function removePollingGetMessages ({ state, commit }) {
  ...
  Vue.$logger.info(`[pluginTraffic]removePollingGetMessages`)
}
async function getExportData ({ state, commit }, { from, to }) {
  ...
  try {
    const params = { data: { from, to } }
    const resp = await Vue.connector.gw.getPluginsPackets(state.active, params)
    Vue.$logger.info(`[pluginTraffic]getExportData`)
    commit('reqStart', { endpoint: 'getPluginsPackets', ids: state.active, params })
    messages = get(resp, 'data.result', [])
  } catch (e) {
    console.log('------------- 6')
