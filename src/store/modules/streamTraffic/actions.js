...
async function initTime ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getStreamsPackets(state.active, params)
    Vue.$logger.info(`[streamTraffic]getMessages`)
    commit('reqStart', { endpoint: 'getStreamsPackets', ids: state.active, params })
...
async function getMessagesTail ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getStreamsPackets(state.active, params)
    Vue.$logger.info(`[streamTraffic]getMessagesTail`)
    commit('reqStart', { endpoint: 'getStreamsPackets', ids: state.active, params })
    const messages = get(resp, 'data.result', [])
    messages.reverse()
    commit('setMessages', messages)
...
async function getMessagesNext ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getStreamsPackets(state.active, params)
    Vue.$logger.info(`[streamTraffic]getMessagesNext`)
    commit('reqStart', { endpoint: 'getStreamsPackets', ids: state.active, params })
...
async function getMessagesPrev ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getStreamsPackets(state.active, params)
    Vue.$logger.info(`[streamTraffic]getMessagesPrev`)
    commit('reqStart', { endpoint: 'getStreamsPackets', ids: state.active, params })
    messages = get(resp, 'data.result', [])
    messages.reverse()
...
function pollingGetMessages ({ state, commit }) {
  mesagesPollingId = setInterval(async () => {
    try {
      ...
      const resp = await Vue.connector.gw.getStreamsPackets(state.active, params)
      Vue.$logger.info(`[streamTraffic]pollingGetMessages`)
      commit('reqStart', { endpoint: 'getStreamsPackets', ids: state.active, params })
      const messages = get(resp, 'data.result', [])
      commit('setMessagesAppend', messages)
    } catch (e) {
...
function removePollingGetMessages ({ state, commit }) {
  ...
  Vue.$logger.info(`[streamTraffic]removePollingGetMessages`)
}
async function getExportData ({ state, commit }, { from, to }) {
  ...
  try {
    const params = { data: { from, to } }
    const resp = await Vue.connector.gw.getStreamsPackets(state.active, params)
    Vue.$logger.info(`[streamTraffic]getExportData`)
    commit('reqStart', { endpoint: 'getStreamsPackets', ids: state.active, params })
    messages = get(resp, 'data.result', [])
  } catch (e) {
    console.log('------------- 6')
