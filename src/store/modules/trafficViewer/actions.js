...
async function getMessages ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getChannelsTrafficEvents(state.active, state.ident, { data: { from: Math.floor(state.from / 1000), to: Math.floor(to / 1000), count: state.limit } })
...
async function getMessagesTail ({ state, commit }) {
  ...
  try {
    const resp = await Vue.connector.gw.getChannelsTrafficEvents(state.active, state.ident, { data: { count: state.limit, reverse: true } })
    const messages = get(resp, 'data.result', [])
    messages.reverse()
    commit('setMessages', messages)
  } catch (e) {
...
async function getMessagesNext ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getChannelsTrafficEvents(state.active, state.ident, { data: { count: state.limit, from, to: Math.floor(state.to / 1000) } })
    const messages = get(resp, 'data.result', [])
...
async function getMessagesPrev ({ state, commit }) {
  ...
  try {
    ...
    const resp = await Vue.connector.gw.getChannelsTrafficEvents(state.active, state.ident, { data: { count: state.limit, from: Math.floor(state.from / 1000), to, reverse: true } })
    const messages = get(resp, 'data.result', [])
    messages.reverse()
...
function pollingGetMessages ({ state, commit }) {
  mesagesPollingId = setInterval(async () => {
    try {
      ...
      const resp = await Vue.connector.gw.getChannelsTrafficEvents(state.active, state.ident, { data: { from, to } })
      const messages = get(resp, 'data.result', [])
      commit('setMessagesAppend', messages)
    } catch (e) {
      commit('reqFailed', e)
...
async function getDevicePreview ({ state }, device) {
  let messages = []
  try {
    const resp = await Vue.connector.gw.getChannelsTrafficEvents(state.active, device.ident, { data: { count: 20, reverse: true } })
