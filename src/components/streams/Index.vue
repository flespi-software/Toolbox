<template>
  <div>
    ...
    <template v-else>
      <q-toolbar slot="header" color="dark">
        <q-item class="no-padding" style="max-width: 50%">
          <q-item-main>
            ...
            <q-item-tile label class="ellipsis overflow-hidden" :style="{maxWidth: '140px'}" v-if="selectedItem.name">{{selectedItem.name}}</q-item-tile>
      ...
      />
      <div class="text-center" style="font-size: 1.5rem; margin-top: 30px; color: white" v-if="!isCustomer || selectedItem.deleted">Nothing to show by stream &#171{{selectedItem.name}}&#187 <div style="font-size: 0.9rem">or you haven`t access</div></div>
    </template>
  </div>
</template>
