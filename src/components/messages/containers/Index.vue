<template>
  <div>
    ...
    >
      ...
      <q-btn slot="after-datetime" @click="refresh" icon="mdi-refresh" flat round dense />
    </virtual-scroll-list>
  </div>
</template>

<script>
import { VirtualScrollList, containersMessagesModule } from 'qvirtualscroll'
import Vue from 'vue'
...
export default {
  ...
  computed: {
    ...
    selectedMessagesTimestamps () {
      ...
      if (this.selected.length) {
        messages = this.selected.map(index => this.messages[index].key)
      }
      return messages
    },
    hasNewMessages: {
  ...
  methods: {
    ...
    scrollToTimestamp (timestamp) {
      const scrollIndex = this.messages.findIndex((message) => timestamp <= message['key'])
    ...
    async init () {
      if (!this.$store.state[this.moduleName]) {
        this.$store.registerModule(this.moduleName, containersMessagesModule({ Vue, LocalStorage: this.$settingsStorage, name: { name: this.moduleName, lsNamespace: 'flespi-toolbox-settings.cols' }, errorHandler: (err) => { this.$store.commit('reqFailed', err) } }))
      } else {
        this.$store.commit(`${this.moduleName}/clear`)
      }
      this.$store.commit(`${this.moduleName}/setSortBy`, 'key')
    ...
    initSelectedByTimestamps (selected) {
      if (selected) {
        const { indexes, messages } = this.messages.reduce((res, message, index) => {
          if (selected.includes(message.key)) {
