...
export default {
  ...
  computed: {
    values () {
      ...
      Object.keys(this.item.params).forEach((propName) => {
        if (!vals[propName]) {
          if (propName.indexOf('image.bin.') !== -1) {
            vals.etc.value += `${propName}: <binary image>`
          } else {
            vals.etc.value += `${propName}: ${JSON.stringify(this.item.params[propName])}; `
          }
        }
      })
      return vals
  ...
  methods: {
    ...
    getValueOfProp (prop, item) {
      ...
      let value = propName === 'key' ? item[propName] : item.params[propName]
