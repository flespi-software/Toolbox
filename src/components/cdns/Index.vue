<template>
  <div>
    ...
    <template v-else>
      ...
      />
      <div class="text-center" style="font-size: 1.5rem; margin-top: 30px; color: white" v-if="!isCustomer || selectedItem.deleted">Nothing to show by CDN &#171;{{selectedItem.name}}&#187; <div style="font-size: 0.9rem">or you haven`t access</div></div>
    </template>
  </div>
</template>
...
export default {
  ...
  created () {
    let activeFromLocaleStorage = this.$q.localStorage.get.item('cdns')
    this.$store.dispatch('getCustomer')
      .then(() => {
        this.$store.dispatch('getItems', 'cdns')
          .then(() => {
            this.isInit = true
