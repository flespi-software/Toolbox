...
import pluginTrafficVuexModule from '../../../store/modules/pluginTraffic'
import hexProcessing from '../../../mixins/hexProcessing'
export default {
  props: ['active', 'isVisibleToolbar', 'config', 'pluginCloseble'],
  data () {
    return {
      typeOfHexView: 'hex',
      limit: 1000,
  ...
  created () {
    console.log('----------------------- pluginTraffic')
    if (!this.$store.state[this.moduleName]) {
      this.$store.registerModule(
        this.moduleName,
        pluginTrafficVuexModule
      )
    }
  },
  destroyed () {
